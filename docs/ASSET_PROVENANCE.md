...
### 0.1 卡提希娅 Q 版（本机非商业验证，不入库）

| 属性 | 值 |
|------|-----|
| **源文件** | `model/Q版/_不可进入开源Godot项目/02_仅限本地非商业且禁止再分发/1010-卡提希娅-*.zip` |
| **格式** | `.pmx` + PNG 贴图 |
| **骨骼 / Morph** | 652 bones / 69 morphs |
| **本机导出** | `assets/local_characters/cartethyia/cartethyia.glb`（Git ignored） |
| **许可证边界** | 禁止商业使用和二次配布；仅作用户本机非商业技术验证 |
| **项目状态** | 已接入共享动作、表情、剧本和独立记忆；不进入公开包 |

### 0.2 相里要 Q 版（本机非商业验证，不入库）

| 属性 | 值 |
|------|-----|
| **源文件** | `model/Q版/_不可进入开源Godot项目/02_仅限本地非商业且禁止再分发/1010-相里要-*.zip` |
| **格式** | `.pmx` + PNG 贴图 |
| **骨骼 / Morph** | 495 bones / 70 morphs |
| **本机导出** | `assets/local_characters/xiangliyao/xiangliyao.glb`（Git ignored） |
| **许可证边界** | 禁止商业使用和二次配布；仅作用户本机非商业技术验证 |
| **项目状态** | 已接入共享动作、表情、剧本和独立记忆；不进入公开包 |
...
| **许可证** | `license: unknown / private prototype only` |
| **来源渠道** | 本地文件 |
| **本机运行路径** | `assets/local_characters/jue/`（整个目录 Git ignored） |
| **状态** | 本机可选加载；公开检出使用无第三方资产的占位角色，Agent 身份与行为合同不变 |
