...
- `[DONE S2.5/P2]` 四个住宅房间按世界原点拼接，几何/碰撞常驻、当前房间导航
...
- `[DONE S1.2]` 四房间改为 Manifest 驱动的独立三灯光组，仅活动房间启用；
...
- `[DONE D2.2/C8.3/S1.2]` 剧情 Beat 自动生成单人/双主体镜头，平滑切入和
  ...
  第一人称模式保持玩家控制。
- `[NEXT]` 补浇水手部 IK/倾倒粒子音效，并完成双角色共同看电视；地点扩展后升级为当前房间 +
  相邻一跳流送。
...
2. `[ACTIVE portable-tool baseline] C9.5 + C4` 浇水已打通真实拿取、携带、
   使用和反馈；继续补 IK、粒子/音效与精确归位。
