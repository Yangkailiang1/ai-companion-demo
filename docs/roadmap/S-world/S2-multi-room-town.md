...
- `[DONE physical door baseline]` 每个活动入口具有带碰撞的 `AnimatableBody3D`
  门扇；玩家或角色进入前置感应区时先开门，跨过独立门槛区后才切换语义房间，
  到达侧保持开启后缓慢关闭。非活动房间的成对门碰撞会停用，避免双层门阻挡。
- `[NEXT]` 增加门把手动作、真实走廊和第三人称玩家表现。
...
- `[NEXT]` 地点扩展到庭院/街道后改为“当前 + 相邻一跳”加载，为未加载居民运行
  更完整的任务剩余时长、关系事件和跨地点资源状态。
